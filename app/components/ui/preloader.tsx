...
const Preloader: React.FC<{ onComplete: () => void }> = ({ onComplete }) => {
  ...
  useEffect(() => {
    const totalTime = 400 + 3000 + 600;
    const timer = setTimeout(() => setFadeOut(true), totalTime);
    return () => clearTimeout(timer);
  }, []);

  const handleTransitionEnd = useCallback(
    (e: React.TransitionEvent) => {
      if (fadeOut && e.propertyName === "opacity") {
        onComplete();
      }
    },
    [fadeOut, onComplete]
