...
const CombinedDarkVeilWaves: React.FC<CombinedProps> = ({
  // DarkVeil defaults
  hueShift = 345,
  noiseIntensity = 0,
  scanlineIntensity = 0,
  ...
  useEffect(() => {
    ...
    wavesCtxRef.current = canvas.getContext("2d");
  ...
  return (
    <div
      ...
      className={className}
    >
      {/* DarkVeil canvas - background layer */}
      <canvas
        ref={darkVeilRef}
      ...
      {/* Waves canvas - overlay layer with blending */}
      <canvas
        ref={wavesCanvasRef}
        className="absolute top-0 left-0 w-full h-full block"
        style={{
          zIndex: 2,
          mixBlendMode: waveBlendMode as any,
          opacity: waveOpacity,
        }}
      />
    </div>
...
export default CombinedDarkVeilWaves;
